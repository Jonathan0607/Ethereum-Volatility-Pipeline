...
std::map<std::string, double> simulate_gbm(double current_price, double predicted_vol, int num_sims = 10000, int steps = 24) {
...
PYBIND11_MODULE(monte_carlo, m) {
    m.def("simulate_gbm", &simulate_gbm, "Runs GBM paths and returns 95% CI",
          py::arg("current_price"), py::arg("predicted_vol"), py::arg("num_sims") = 10000, py::arg("steps") = 24);
}
