...
def analyze_drawdowns():
    ...
    # Trace drawdowns
    cum_strat = test_df['cumulative_strategy']
    peak = cum_strat.cummax()
    dd = (cum_strat - peak) / peak
    test_df['drawdown'] = dd
    
    # Find local minima of drawdown
    # We want to identify the largest drawdown periods
    dd_threshold = -0.05
    in_dd = False
    dd_start = None
    dd_min = 0.0
    ...
    drawdown_events = sorted(drawdown_events, key=lambda x: x['max_dd'])
    
    print("\n=== TOP 5 LARGEST DRAWDOWN EVENTS ===")
    for i, ev in enumerate(drawdown_events[:5]):
        print(f"\n{i+1}. Drawdown: {ev['max_dd']*100:.2f}%")
        print(f"   Start: {ev['start']} | Max DD Time: {ev['min_time']} | End: {ev['end']}")
        
        # Analyze the market and strategy state around the Max DD time
        sub_df = test_df.loc[ev['start']:ev['min_time']]
        print(f"   Market return during drawdown: {(sub_df['cumulative_market'].iloc[-1]/sub_df['cumulative_market'].iloc[0] - 1)*100:.2f}%")
        print(f"   Positions held during drawdown:")
        print(sub_df['active_position'].value_counts())
        
        # Print first few rows of the drawdown start to see how it was triggered
        start_idx = test_df.index.get_loc(ev['start'])
        pre_start_df = test_df.iloc[max(0, start_idx-24):start_idx+24]
        # Check if there was a big bullish move before the start of the drawdown
        # e.g., 48h market return before start
        price_at_start = test_df['close'].loc[ev['start']]
        price_48h_before = test_df['close'].iloc[max(0, start_idx-48)]
        pre_48h_ret = (price_at_start / price_48h_before) - 1
        print(f"   Market return 48h prior to drawdown start: {pre_48h_ret*100:.2f}%")
