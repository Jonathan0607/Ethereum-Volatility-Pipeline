...
def run_combined_simulation(df, sl_pct, max_hours):
    ...
    for i in range(len(test_df)):
        ...
        if not np.isnan(r_min) and close < r_min:
            ...
            if current_s_breakout == -1.0:
                ...
                if max_hours is not None and short_hold_duration >= max_hours:
                    is_exit = True
                
                # Option 2: Hard Stop Loss
                if sl_pct is not None and short_entry_price > 0.0:
                    loss = (close - short_entry_price) / short_entry_price
                    if loss > sl_pct:
                        is_exit = True
...
def main():
    ...
    print("Evaluating combined 4% SL and 72h Time Stop...")
    ret, sharpe, dd, slice_ret = run_combined_simulation(test_df, 0.04, 72)
    print(f"Combined (4% SL + 72h Stop) -> Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")
