...
def run_simulation_bypass(df, min_short_weight):
    ...
    # Apply MIN SHORT WEIGHT floor if price is below EMA 200 and breakout signal is active
    # This prevents the HMM from scaling down short positions during macro downtrends
    is_below_ema = (test_df['close'] < test_df['ema_200']).values
    w_breakout = np.where(is_below_ema & (s_breakout_arr == -1.0), np.maximum(w_breakout, min_short_weight), w_breakout)
...
def main():
    ...
    print("Evaluating short weight floors when close < ema_200:")
    
    # Baseline (no floor = 0)
    ret, sharpe, dd, slice_ret = run_simulation_bypass(test_df, 0.0)
    print(f"Floor 0.0 (Baseline) -> Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")
    
    for floor in [0.2, 0.4, 0.5, 0.6, 0.8, 1.0]:
        ret, sharpe, dd, slice_ret = run_simulation_bypass(test_df, floor)
        print(f"Floor {floor:.2f} -> Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")
