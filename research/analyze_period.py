...
def analyze_period():
    ...
    if slice_df.empty:
        print("Slice is empty! Check index format.")
        print("Index example:", test_df.index[0])
        return

    # Calculate returns for this slice
    market_ret = (slice_df['close'].iloc[-1] / slice_df['close'].iloc[0]) - 1
    strat_ret = (slice_df['cumulative_strategy'].iloc[-1] / slice_df['cumulative_strategy'].iloc[0]) - 1
    
    print(f"=== PERIOD ANALYSIS: {start_date.date()} to {end_date.date()} ===")
    print(f"Market Return:   {market_ret * 100:.2f}%")
    print(f"Strategy Return: {strat_ret * 100:.2f}%")
    
    # Calculate Sharpe in this slice
    strat_mean = slice_df['strategy_returns'].mean() * 24 * 365
    strat_std = slice_df['strategy_returns'].std() * np.sqrt(24 * 365)
    sharpe = strat_mean / (strat_std + 1e-9)
    print(f"Sharpe in slice: {sharpe:.4f}")
    
    # Position statistics in slice
    print("\nPositions held during this period:")
    print(slice_df['active_position'].value_counts())
    
    # Identify trades in slice
    # Check what trades were made
    trade_starts = slice_df[slice_df['active_position'] != slice_df['active_position'].shift(1)]
    print(f"\nPosition changes in slice (total {len(trade_starts)}):")
    for idx, row in trade_starts.head(20).iterrows():
        print(f"Time: {idx} | Close: {row['close']:.2f} | Target Position: {row['active_position']:.4f} | prob_high_vol: {row['prob_high_vol']:.4f} | z_score: {row['z_score']:.4f}")
