...
def run_simulation_dip_filter(df, filter_type, filter_val):
    ...
    for i in range(len(test_df)):
        ...
        if not np.isnan(r_min) and close < r_min:
            ...
            if trend_active:
                # Evaluate the dip filter
                allow_short = True
                
                # Filter 1: EMA Distance
                if filter_type == 'ema_dist':
                    # close must not be below ema_200 * (1 - filter_val)
                    if close < ema_200s[i] * (1.0 - filter_val):
                        allow_short = False
                        
                # Filter 2: Local Z-score
                elif filter_type == 'z_score':
                    if z_score < filter_val:
                        allow_short = False
                        
                # Filter 3: Recent 24h Return
                elif filter_type == 'ret_24h':
                    if i >= 24:
                        ret_24h = (close / closes[i-24]) - 1
                        if ret_24h < -filter_val:
                            allow_short = False
            ...
            if current_s_breakout == -1.0:
                ...
                # Include the 72h Time Stop since it's the preferred Option C
...
def main():
    ...
    # Baseline Option C (which has 72h Stop)
    print("Baseline Option C (72h Time Stop, no dip filter):")
    ret, sharpe, dd, slice_ret = run_simulation_dip_filter(test_df, 'none', 0)
    print(f"   Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")
    
    # 1. EMA Distance Filter on Shorts
    print("\nEvaluating EMA Distance Filter: Block short entry if close < ema_200 * (1 - X)")
    for dist in [0.03, 0.05, 0.08, 0.10, 0.12]:
        ret, sharpe, dd, slice_ret = run_simulation_dip_filter(test_df, 'ema_dist', dist)
        print(f"   Dist {dist*100:.1f}% -> Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")

    # 2. Local Z-Score Filter on Shorts
    print("\nEvaluating Local Z-score Filter: Block short entry if z_score < Z")
    for z_limit in [-1.0, -1.5, -2.0, -2.5]:
        ret, sharpe, dd, slice_ret = run_simulation_dip_filter(test_df, 'z_score', z_limit)
        print(f"   Z-limit {z_limit:.2f} -> Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")

    # 3. 24h Return Filter on Shorts
    print("\nEvaluating 24h Return Filter: Block short entry if 24h return < -Y")
    for ret_limit in [0.05, 0.08, 0.10, 0.12]:
        ret, sharpe, dd, slice_ret = run_simulation_dip_filter(test_df, 'ret_24h', ret_limit)
        print(f"   24h Return < -{ret_limit*100:.1f}% -> Full Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Slice Return: {slice_ret:.2f}%")
