...
def test_strategy_with_filter(df, short_entry_filter_fn):
    # Load parameters
    ...
    for i in range(len(test_df)):
        ...
        if not np.isnan(r_min) and close < r_min:
            ...
            if trend_active:
                # Evaluate custom filter function
                allow_short = short_entry_filter_fn(test_df, i)
                if allow_short:
...
def run_all_tests():
    ...
    # Add columns that are pre-calculated on the whole dataset
    test_df['ema_200'] = df['ema_200'].loc[test_df.index]
    test_df['ema_50'] = df['ema_50'].loc[test_df.index]
    
    print("Baseline:")
    # Baseline: no extra filter on shorts
    ret, sharpe, dd, nl, ns = test_strategy_with_filter(test_df, lambda df, i: True)
    print(f"   Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Longs: {nl} | Shorts: {ns}")
    
    # Filter 1: EMA 200 Trend Filter
    # Only short if close is below EMA 200 (macro downtrend)
    print("\nFilter: close < ema_200 (Only Short in Macro Downtrend)")
    def f_ema200(df, i):
        return df['close'].iloc[i] < df['ema_200'].iloc[i]
    ret, sharpe, dd, nl, ns = test_strategy_with_filter(test_df, f_ema200)
    print(f"   Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Longs: {nl} | Shorts: {ns}")

    # Filter 2: EMA 50 Trend Filter
    # Only short if close is below EMA 50
    print("\nFilter: close < ema_50 (Only Short in Medium-Term Downtrend)")
    def f_ema50(df, i):
        return df['close'].iloc[i] < df['ema_50'].iloc[i]
    ret, sharpe, dd, nl, ns = test_strategy_with_filter(test_df, f_ema50)
    print(f"   Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Longs: {nl} | Shorts: {ns}")

    # Filter 3: Price must not have risen too much in the last 48h (e.g. < +5%)
    print("\nFilter: 48h price return < threshold")
    for thresh in [0.02, 0.05, 0.08, 0.10]:
        def f_ret48h(df, i, t=thresh):
            if i < 48:
                return True
            ret_48h = (df['close'].iloc[i] / df['close'].iloc[i-48]) - 1
            return ret_48h < t
        ret, sharpe, dd, nl, ns = test_strategy_with_filter(test_df, f_ret48h)
        print(f"   Thresh {thresh*100:.1f}% -> Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Longs: {nl} | Shorts: {ns}")

    # Filter 4: Price must not have risen too much in the last 24h
    print("\nFilter: 24h price return < threshold")
    for thresh in [0.02, 0.05, 0.08, 0.10]:
        def f_ret24h(df, i, t=thresh):
            if i < 24:
                return True
            ret_24h = (df['close'].iloc[i] / df['close'].iloc[i-24]) - 1
            return ret_24h < t
        ret, sharpe, dd, nl, ns = test_strategy_with_filter(test_df, f_ret24h)
        print(f"   Thresh {thresh*100:.1f}% -> Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Longs: {nl} | Shorts: {ns}")

    # Filter 5: Combine EMA 200 filter and 24h return filter
    print("\nFilter: close < ema_200 AND 24h price return < +5%")
    def f_comb(df, i):
        cond1 = df['close'].iloc[i] < df['ema_200'].iloc[i]
        cond2 = True
        if i >= 24:
            ret_24h = (df['close'].iloc[i] / df['close'].iloc[i-24]) - 1
            cond2 = ret_24h < 0.05
        return cond1 and cond2
    ret, sharpe, dd, nl, ns = test_strategy_with_filter(test_df, f_comb)
    print(f"   Return: {ret:.2f}% | Sharpe: {sharpe:.4f} | Max DD: {dd:.2f}% | Longs: {nl} | Shorts: {ns}")
